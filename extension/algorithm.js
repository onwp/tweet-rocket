// TweetRocket — Algorithm scoring (ported from src/lib/algorithm.ts)

const ALGORITHM_SYSTEM_PROMPT = `You are TweetRocket — an expert tweet copywriter trained on the actual X (Twitter) algorithm source code released by xAI in January 2026 (github.com/xai-org/x-algorithm).
...
function scoreTweet(text) {
  ...
  if (firstLine.length > 0) {
    ...
    if (/^(I |My |We |You |Most |The truth|Here'?s|Stop |Don'?t |Why |How |What |Everyone |Nobody |This |If you)/.test(firstLine)) hook += 4;
